...
| RF-003 | P1 | Autorização/privacidade | Pacientes/atendimentos têm operações públicas e POST de atendimento aceita qualquer `idUsuario` | `SecurityConfig.java:42-44` no baseline; `AtendimentoRequestDTO.java:8`; `AtendimentoService.java:39` | Aprovar matriz de acesso; proteger dados sensíveis; derivar usuário do `SecurityContext` quando esse for o significado correto | Mostra autorização além de “ter JWT” | Pode mudar comportamento do front-end desconhecido; requer decisão de papéis | EM_ANDAMENTO |
| RF-004 | P1 | API/segurança | Cadastro recebe/retorna `Usuario`, incluindo hash BCrypt serializável | `AuthController.java:27-30`; `Usuario.java:39-41` | Criar DTOs mínimos de cadastro/resposta, nunca retornar senha/hash, responder 201 e tratar e-mail duplicado | Contrato seguro e explícito | Ajusta resposta pública; verificar consumidor | PROPOSTO |
...
| RF-006 | P1 | Validação | Auth, médico e atendimento não têm validação efetiva; DTOs validados de paciente/médico estão sem uso | `AutenticacaoDTO`; `Medico`; `AtendimentoRequestDTO`; request DTOs não referenciados | Definir contratos reais, aplicar constraints/`@Valid` e validar IDs/data/sala/e-mails | Evita 500 e protege invariantes de entrada | Não inventar CPF/CRM/agenda sem requisito aprovado | PROPOSTO |
| RF-007 | P1 | Erros/API | Handler cobre apenas Bean Validation; ausências e integridade viram respostas inconsistentes/500 | `GlobalExceptionHandler`; `PacienteService.java:34-35`; `MedicoService.java:30`; `AtendimentoService.java:41-42` | Criar exceções focadas e handler uniforme para 400/404/409; preservar detalhes úteis sem stack trace | API defensável e previsível | Definir formato e compatibilidade antes | PROPOSTO |
| RF-008 | P1 | Testes | Nenhum teste estava ativo; build verde não verificava comportamento | teste integralmente comentado no baseline; Surefire executava zero testes | Criar baseline de contexto com profile isolado e testes de auth, segurança, validação e fluxos CRUD prioritários | Principal evidência de evolução técnica | Testes podem revelar bugs; registrar antes de corrigir | EM_ANDAMENTO |
