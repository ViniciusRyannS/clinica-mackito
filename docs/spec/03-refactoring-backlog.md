...
| RF-003 | P1 | Autorização/privacidade | Pacientes/atendimentos eram públicos e POST aceitava qualquer `idUsuario` | Baseline de `SecurityConfig`, `AtendimentoRequestDTO` e `AtendimentoService` | Proteger dados, criar roles, derivar registrador do contexto e adicionar ownership | Mostra autorização além de “ter JWT” | Autoagendamento permanece em decisão separada | CONCLUÍDO |
...
| RF-009 | P1 | Execução/documentação | Não havia README, bootstrap do MySQL nem exemplos de configuração/comandos | Baseline da raiz e falha de startup | Documentar execução, migrations, bootstrap admin e Postman | Recrutador consegue executar e avaliar | Demo hospedada ainda falta | EM_ANDAMENTO |
...
| RF-011 | P2 | Persistência | `ddl-auto=update` não oferecia histórico reproduzível do schema | Baseline de `application.properties`; ausência de migrations | Introduzir Flyway e usar Hibernate `validate` no MySQL | Reprodutibilidade e disciplina de banco | V1 pressupõe database novo; compatibilidade MySQL automatizada ainda falta | CONCLUÍDO |
| RF-012 | P2 | Domínio/dados | Não havia constraints de unicidade para CPF/CRM/e-mails | Baseline de entities e schema gerado | Validar na aplicação e criar constraints no schema versionado | Integridade de dados baseada em regra aprovada | Médico não possui e-mail no modelo efetivo | CONCLUÍDO |
...
| RF-019 | P1 | Agenda | O modelo representava somente a data e não detectava conflito de disponibilidade | `Atendimento.dataAtendimento`; ADR-007 | Modelar horário/duração e impedir sobreposição de médico/sala | Torna a confirmação de agenda confiável | Expediente ainda não definido | CONCLUÍDO |
| RF-020 | P2 | Concorrência | A validação de sobreposição ocorre na aplicação e duas confirmações realmente simultâneas podem competir | `DisponibilidadeAgendaService` | Avaliar lock pessimista ou outra serialização ao preparar ambiente multi-instância | Explicita limite de consistência da versão de portfólio | Depende do banco e da arquitetura de execução | PROPOSTO |
